...
macro_rules! cprintln {
    ($color:expr, $($arg:tt)*) => {
        $crate::util::cprintln::cprintln_imp($color, &format!($($arg)*))
    };
}
pub use cprintln;
...
fn process_markdown(text: &str, base_color: (u8, u8, u8)) -> String {
    ...
    // A couple convenience behaviors since this isn't really Markdown
    ...
    let root = parse_document(&arena, text.as_str(), &options);

    let mut result = String::new();
    collect_text(root, &mut result, base_color);
    result
}

fn collect_text<'a>(node: &'a AstNode<'a>, result: &mut String, base_color: (u8, u8, u8)) {
    ...
    match &node.data.borrow().value {
        ...
        | NodeValue::Paragraph => {
            // For all these container nodes, just process their children
            for child in node.children() {
                collect_text(child, result, base_color);
            }
        }
        NodeValue::List(_) => {
            let mut i = 1;
            for item in node.children() {
                ...
                i += 1;
                collect_text(item, result, base_color);
            }
        }
        NodeValue::Link(link_data) => {
            // Special handling for links: treat the URL as a color
            let link_text = extract_text_content(node);
            let url = &link_data.url;
            let link_color = parse_color(url);
            let reset_sequence = ansi_rgb(base_color.0, base_color.1, base_color.2);
            result.push_str(&format!(
                "{}{}{}",
                ansi_rgb(link_color.0, link_color.1, link_color.2),
                link_text,
                reset_sequence
            ));
        }
        NodeValue::Text(text) => {
            ...
            // Remove escape characters on escaped whitespace
            if let Some(stripped) = text.strip_prefix('\\') {
                if stripped.chars().next().map_or(false, |c| c.is_whitespace()) {
                    text = stripped.to_string();
                }
            }
            // Remove escape characters before numbers or whitespace using regex
        ...
        NodeValue::CodeBlock(code_block) => {
            let content = code_block.literal.trim_end();
            result.push_str(&content);
        }
        NodeValue::HtmlInline(html) => {
            match html.as_str() {
                "<br/>" => result.push('\n'),
                _ => result.push_str(html.as_str()),
            };
        }
        NodeValue::HtmlBlock(html) => {
            result.push_str(&html.literal);
        }
        ...
        NodeValue::Image(_) => {
            // For images, just extract the alt text
            let alt_text = extract_text_content(node);
            result.push_str(&alt_text);
        }
        _ => {
            // For any other node types, just process children
            for child in node.children() {
                collect_text(child, result, base_color);
            }
        }
...
fn extract_text_recursive<'a>(node: &'a AstNode<'a>, result: &mut String) {
    match &node.data.borrow().value {
        NodeValue::Text(text) => {
